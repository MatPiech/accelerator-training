...
def check_requires_grad(all_layers: list[torch.nn.Module], frozen_layer_num: int):
    ...
    for idx, layer in enumerate(all_layers):
        ...
        if idx <= frozen_layer_num:
            print(f'\t{(str(idx+1)+".").ljust(4)} {(layer.__class__.__name__+":").ljust(25)} {layer_requires_grad}\t->\tfrozen')
        elif not layer_requires_grad:
            print(f'\t{(str(idx+1)+".").ljust(4)} {(layer.__class__.__name__+":").ljust(25)} {layer_requires_grad}\t->\tnot trainable')
        else:
            print(f'\t{(str(idx+1)+".").ljust(4)} {(layer.__class__.__name__+":").ljust(25)} {layer_requires_grad}\t->\ttrainable')
...
class Model(torch.nn.Module):
    def __init__(self, model_name: str, num_classes: int, channels: int, frozen_layer_num: int, img_size: Optional[int] = None):
