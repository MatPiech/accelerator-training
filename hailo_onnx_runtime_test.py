...
def main(model_path: Path, provider: str, iterations: int):
    ep_list = ["HailoExecutionProvider"] if provider == "hailo" else ["CPUExecutionProvider"]
    ...
    input_dict = {si.name: np.random.randn(1, *si.shape[1:]).astype(np.float32) for si in session.get_inputs()}
